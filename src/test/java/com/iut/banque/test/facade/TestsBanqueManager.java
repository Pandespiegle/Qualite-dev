...
public class TestsBanqueManager {
	...
	public void TestCreationDunClient() {
		try {
			bm.loadAllClients();
			bm.createClient("t.test1", "password", "test1nom", "test1prenom", "testmail","test town", true, "4242424242");
	...
	public void TestCreationDunClientAvecDeuxNumerosDeCompteIdentiques() {
		try {
			bm.loadAllClients();
			bm.createClient("t.test1", "password", "test1nom", "test1prenom","testmail", "test town", true, "0101010101");
			fail();
		} catch (IllegalOperationException e) {
		} catch (Exception te) {
			te.printStackTrace();
